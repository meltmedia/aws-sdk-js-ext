...
const
  _ = require('lodash'),
  ZSchema = require('z-schema').default,
  error = require('./error'),
...
const
  ...
  DEFAULT_VALIDATOR = ZSchema.create(_.merge(DEFAULT_AWS_EXT_CONFIG, config.awsext).schema.options);
  ...
  validate(data, schema) {
    return this._validate(data, schema)
      .catch(err => {
        ...
        throw new error.ValidationError((schema || {}).id, err);
      });
  }
