...
const
  { KMSClient, GenerateDataKeyCommand, DecryptCommand } = require('@aws-sdk/client-kms'),
...
class EncryptionUtil {
   ...
   * @param  {KMSClient} [kms] - The AWS KMS client instance
   * @return {EncryptionUtil}
   */
  constructor(config, kms) {
    config = config || {};
    this.key = config && config.key;
    this.kms = kms || new KMSClient(config.kmsOptions || {});
  }
  ...
  encrypt(data) {
    ...
    return this.kms.send(new GenerateDataKeyCommand(params))
      .then(keys => {
        keys.Plaintext = Buffer.from(keys.Plaintext).toString('hex');
        keys.CiphertextBlob = Buffer.from(keys.CiphertextBlob).toString('hex');
        return keys;
      })
      .then(keys => {
   ...
   * @param  {KMSClient} kms - An AWS KMS client instance.
  ...
  decrypt(encryptedPayload) {
    return Promise.resolve()
      ...
      .then(params => this.kms.send(new DecryptCommand(params)))
      .then(decryptedKey => {
        let key = Buffer.from(decryptedKey.Plaintext).toString('hex');
