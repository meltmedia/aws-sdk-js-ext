...
class SqsConsumer extends SqsBase {
   ...
   * @param {String} messageId - Message id of the SQS message
   * @return {Promise.<*>}
   */
  handle(msgBody, messageId) {
  ...
  _handle(message) {
    ...
    return Promise.resolve(JSON.parse(message.Body))
      .then(messageBody => this._decryptMessage(messageBody, message.MessageId))
      .then(messageBody => this.validateMessage(messageBody, message.MessageId))
      .then(messageBody => this.handle(messageBody, message.MessageId))
