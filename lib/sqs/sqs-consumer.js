...
class SqsConsumer extends events.EventEmitter {
  ...
  constructor(options, handler) {
    ...
    this._getQueueUrl = promisify(this._sqs.getQueueUrl, this._sqs);
    this._createQueue = promisify(this._sqs.createQueue, this._sqs);
    this._deleteMessage = promisify(this._sqs.deleteMessage, this._sqs);
    this._receiveMessage = promisify(this._sqs.receiveMessage, this._sqs);
    this._changeMessageVisibility = promisify(this._sqs.changeMessageVisibility, this._sqs);
  ...
  _handle(message) {
    ...
    return Promise.resolve(JSON.parse(message.Body))
      ...
      .then(messageBody => {
        return this.handle(messageBody);
      })
