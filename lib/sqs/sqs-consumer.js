...
class SqsConsumer extends SqsBase {
  ...
  _poll() {
    ...
    .then(data => this._prepareForScheduledProcessing(data))
    .then(data => {
  ...
  _prepareForScheduledProcessing(data) {
    let scheduler = this.conf.consumer.scheduler;
    if(!scheduler || !scheduler.scheduled) return Promise.resolve(data);

    let now = moment(),
        start = moment(scheduler.start, 'hh:mm:ss'),
        end = moment(start).add(scheduler.duration, 'seconds'),
        nextStart = moment(start),
        timeout = scheduler.maxVisibilityTimeout;
