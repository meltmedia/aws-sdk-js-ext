...
class SqsConsumer extends SqsBase {
  ...
  constructor(options, handler) {
    super(options, DEFAULT_AWS_EXT_CONFIG);
  ...
  _poll() {
    ...
    .then(data => this._prepareForProcessWindow(data))
    .then(data => {
  ...
  _prepareForProcessWindow(data) {
    let schedule = this.conf.consumerSchedule;
    if(!schedule || !schedule.running) return Promise.resolve(data);

    let now = moment(),
        start = moment(schedule.window.start, 'hh:mm:ss'),
        end = moment(schedule.window.end, 'hh:mm:ss'),
        nextStart = moment(start),
        timeout = schedule.maxWaitSeconds;
    ...
    if(now.isAfter(end)) nextStart.add(1, 'days');
    timeout = Math.min(timeout, nextStart.diff(now, 'seconds'));
