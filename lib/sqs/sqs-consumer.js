...
class SqsConsumer extends events.EventEmitter {
  ...
  constructor(options) {
    ...
    this.validator = options.validator || new validation.ValidationUtil();

    let awsext = _.merge(DEFAULT_AWS_EXT_CONFIG, config.awsext);
    let consumersConf = awsext.sqs.consumers;
    this.conf = _.merge(consumersConf.defaults, consumersConf[name], options.conf);
    this._sqs = sqs || new AWS.SQS(this.conf.sqsOptions);
    ...
    this._jobRetrier = new commonUtils.Retrier(this.conf.poll.error.minWait, conf.poll.error.maxWait);
    this._messageRetrier = new commonUtils.Retrier(conf.message.error.minvisibility,
      conf.message.error.maxvisibility);
    this._schemaService = schemaService || init.services.schema;
  }
    ...
    return Promise.resolve(JSON.parse(message.Body))
      .then(messageBody => {
        if(this.conf.schema && this.conf.schema.name) {
          return this._schemaService.validate(messageBody, this.conf.schema.name).then(() => {
            return messageBody;
          });
        }
        return messageBody;
      ...
      .then(messageBody => {
        return this._handleMessageBody(messageBody);
      })
