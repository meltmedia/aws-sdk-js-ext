...
const
  ...
  { AwsQueryProtocol } = require('@aws-sdk/config/protocol');

const QUEUE_DOES_NOT_EXIST_ERRORS = ['QueueDoesNotExist', 'AWS.SimpleQueueService.NonExistentQueue'];
...
class SqsBase extends events.EventEmitter {
  ...
  constructor(options, defaultAwsExtConfig) {
    ...
    let awsext = _.merge({}, DEFAULT_BASE_AWS_EXT_CONFIG, config.awsext, defaultAwsExtConfig);
    this.conf = _.merge({}, awsext.sqs.defaults, awsext.sqs[this.name], options.conf);
    this._sqs = options.sqs || new SQSClient(_.assign({ protocol: AwsQueryProtocol }, this.conf.sqsOptions));
    this._kms = options.kms || new KMSClient(this.conf.kmsOptions);
  ...
  deleteQueue() {
    if(this._queueUrl) {
      return this.sqs.send(new DeleteQueueCommand({QueueUrl: this._queueUrl}))
        .then(resp => {
  ...
  purgeQueue() {
    return this._init()
      .then(() => this.sqs.send(new PurgeQueueCommand({QueueUrl: this._queueUrl})))
      .then(resp => {
  ...
  sendMessage(msgData, sensitiveMsgData) {
    ...
    return this._init()
      ...
      .then(sqsMsg => {
        return this._sqs.send(new SendMessageCommand({
  ...
  _init() {
    if(!this._queueUrl) {
      ...
      return this._sqs.send(new GetQueueUrlCommand({QueueName: this._queueName}))
        .then(resp => {
          ...
          if(QUEUE_DOES_NOT_EXIST_ERRORS.includes(err.name) || QUEUE_DOES_NOT_EXIST_ERRORS.includes(err.code)) {
            return this._sqs.send(new CreateQueueCommand({
              QueueName: this._queueName,
